...
class CloudSqlDataModel
{
    ...
    public function __construct(PDO $pdo)
    {
        ...
        $this->pdo->query("CREATE TABLE IF NOT EXISTS user ($columnText)");
    }
    ...
    public function createuser($book)
    {   
        $names = array_diff_key($book);
        if($names['upw'] == $names["upw2"]){
            $pdo = $this->pdo;
            $stmt = $pdo->prepare("INSERT INTO user(ID, PW) VALUES(:id, :pw)");
            $stmt->bindParam(':id', $names['decide_id1'], PDO::PARAM_STR);
            $stmt->bindParam(':pw', $names['upw'], PDO::PARAM_STR);
            $stmt->execute();
            $pdo = null;
            return 1;
        }
